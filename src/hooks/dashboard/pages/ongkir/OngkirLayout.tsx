...
import { CreateModal } from '@/hooks/dashboard/pages/ongkir/modal/CreateModal'

import { EditModal } from '@/hooks/dashboard/pages/ongkir/modal/EditModal'
...
import { OngkirPropes } from "@/hooks/dashboard/pages/ongkir/types/ongkir"

export default function OngkirLayout() {
    const [items, setItems] = React.useState<OngkirPropes[]>([]);
    ...
    const handleCreate = async (data: {
        ...
        try {
            ...
            const docRef = await addDoc(collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_ONGKIR as string), {
                desa: data.desa,
                price: data.price,
                createdAt: now,
                updatedAt: now
    ...
    const handleUpdate = async (data: {
        ...
        try {
            ...
            await updateDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_ONGKIR as string, data.id), {
                desa: data.desa,
                price: data.price,
    ...
    const handleDelete = async (id: string) => {
        try {
            setIsDeleting(true);
            await deleteDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_ONGKIR as string, id));
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border border-gray-100 p-4 rounded-2xl gap-4">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 pb-4">
                        ...
                        <h1 className="text-3xl font-bold tracking-tight">Ongkir</h1>
                    </div>
                        ...
                        <TableBody>
                            {items.map((item) => (
                                <TableRow key={item.id}>
                                    <TableCell className="font-medium">{item.desa}</TableCell>
                                    <TableCell>{formatPriceWithSymbol(item.price)}</TableCell>
