...
import { aboutProps } from "@/hooks/dashboard/pages/about/types/about"

export default function AboutLayout() {
    const [items, setItems] = React.useState<aboutProps[]>([]);
    ...
    const fetchItems = async () => {
        try {
            const q = query(
                collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_ABOUT_CONTENT as string),
    ...
    const handleCreate = async (data: {
        ...
        try {
            ...
            const docRef = await addDoc(collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_ABOUT_CONTENT as string), {
                title: data.title,
    ...
    const handleUpdate = async (data: {
        ...
        try {
            ...
            await updateDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_ABOUT_CONTENT as string, data.id), {
                title: data.title,
                description: data.description,
                imageUrl: imageUrl,
                button: data.button,
    ...
    const handleDelete = async (id: string) => {
        try {
            setIsDeleting(true);
            await deleteDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_ABOUT_CONTENT as string, id));
